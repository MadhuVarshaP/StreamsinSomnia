...
export function MintNftForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const invalidAddresses = splits.filter(
      (split) => !isValidAddress(split.address)
    );
    ...
    try {
      ...
      const response = await mintWithCustomSplitter(
      ...
      toast.success("NFT minted successfully and listed for sale!", {
        id: "mint",
      });

      // Dispatch custom event for real-time updates
      window.dispatchEvent(
        new CustomEvent("nftMinted", {
          detail: {
            owner: address,
      ...
      // Also dispatch to transaction history
      ...
      if (err instanceof Error) {
        if (err.message.includes("IPFS")) {
          ...
          errorMessage =
            "Failed to deploy royalty splitter. Please check your wallet and try again.";
        } else if (err.message.includes("mint")) {
          errorMessage =
            "Failed to mint NFT. Please ensure you have enough funds and try again.";
          ...
          errorMessage =
            "NFT minted but failed to list for sale. You can list it manually later.";
      ...
      <Card className="relative overflow-hidden border-2 border-lime-500/20 bg-gradient-to-br from-lime-500/10 via-cyan-500/5 to-orange-500/10 backdrop-blur-xl text-[#f5eada] shadow-2xl">
        ...
        <CardHeader className="relative z-10 pb-6">
          <div className="flex items-center gap-3 mb-2">
            ...
            <CardTitle className="text-2xl font-bold text-[#f5eada]">
              Mint New NFT
            </CardTitle>
          </div>
          <p className="text-[#f5eada]/70">
            Create your NFT with flexible royalty splits and start earning from
            streams
          </p>
        </CardHeader>
        ...
        <form onSubmit={handleSubmit}>
          <CardContent className="relative z-10 space-y-6">
            ...
            >
              <div className="grid gap-3">
                <Label
                  htmlFor="nft-name"
                  className="text-sm font-medium text-[#f5eada]/90 flex items-center gap-2"
                >
              ...
              <div className="grid gap-3">
                <Label
                  htmlFor="nft-desc"
                  className="text-sm font-medium text-[#f5eada]/90"
                >
                  Description
                </Label>
                <Textarea
            ...
            >
              ...
              <p className="text-xs text-[#f5eada]/60">
                Royalty percentage for secondary sales (e.g., 10% = 1000 basis
                points)
              </p>
            </motion.div>
            ...
            >
              ...
              <div className="space-y-3">
                {splits.map((s, i) => (
                  ...
                  >
                    <div className="text-xs text-[#f5eada]/60 font-medium">
                      Recipient {i + 1}
                    </div>
              ...
              >
                <span className="text-sm text-[#f5eada]/70">
                  Total Percentage
                </span>
          ...
          <CardFooter className="relative z-10 flex justify-end pt-6">
            <motion.div
              whileHover={{
                scale: isMintPending || isMintConfirming ? 1 : 1.02,
              }}
            ...
            >
              <Button
                ...
                disabled={
                  ...
                  !isConnected ||
                  totalPercent !== 100 ||
                  splits.some((s) => !isValidAddress(s.address))
                }
