...
contract RoyaltySplitterFactory {
    ...
    function createSplitter(RoyaltySplitter.Share[] calldata shares, address token) external payable returns (address splitter) {
        ...
        splitter = address(new RoyaltySplitter{value: msg.value}(shares, token));
